...
- ViewStub的`layoutParameters`应用：如果 ViewStub 设置了LayoutParams，则使用LayoutParams。否则，忽略。
- `mInflatedViewRef`通过`弱引用`形式,建立ViewStub与加载的View的联系
...
- `android:inflatedId` 加载的布局被重写的id
- 加载对象一个 layout，而不是一个 view
- ViewStub中不能嵌套一个 Merge 标签
...
不同：  
setVisibility只是在ViewStub第一次延迟初始化时,并且visibility是非GONE 时,调用了 inflate 方法.

	
## 说明
- ViewStub 是一种不可见且大小为0的视图，可以延迟到运行时填充（inflate）布局资源。   
当 ViewStub 设置为`可视`或invoke `inflate()`后，填充布局资源，然后 ViewStub 被填充的视图替代。
