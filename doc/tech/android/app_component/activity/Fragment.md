...
# 1 设计Fragment的目的？
为了适应大屏幕的平板电脑。使用Fragment对UI组件进行分组、模块化管理 => Fragment是可复用的组件。
...
# 2 Fragment 特征
- Fragment 拥有自己的输入事件，有自己的生命周期——受到Activity的生命周期的影响。
- 只有Activity处于活动状态时，才能操作Fragment。
...
## onCreate
- 系统创建Fragment对象后回调该方法

## onCreateView
- 当Fragment绘制界面组件时回调该方法
...
```
# 7 BackStack
FragmentTransaction（Fragment事务）。与数据库事务类似，数据库事务代表对底层数组的多个更新操作。
FragmentTransaction代表了Activity对Fragment执行的多个改变操作:add()/remove()/replace()，最后commit提交事务。
