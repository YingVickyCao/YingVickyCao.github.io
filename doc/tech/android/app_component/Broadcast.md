...
- `LocalBroadcastManager.getInstance(this).registerReceiver(BroadcastReceiver, IntentFilter)` 注册的广播，发送广播的时候must 使用`LocalBroadcastManager.sendBroadcast(Intent);` 否则接收不到广播。   
