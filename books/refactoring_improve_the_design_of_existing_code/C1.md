...
- 代码块俞小，代码的功能就俞容易管理，代码的处理和移动也就俞轻松。  
  => **_每个类不能长于 1000 行，一个方法的长度尽量控制在 50 行内_**
...
| v8      | Movie 类型                                                                                                   | Replace Type Code with State/Strategy(227)<br/> Self Encapsulate(171) <br/>:State 模式 |
| v9      | Movie - getCharge                                                                                            | Move Method(142):State 模式                                                            |
| v10     | Price- getCharge switch                                                                                      | Replace Conditional with Polymorphism(255):State 模式                                  |
| v11     | Movie - getFrequentRenterPoints switch                                                                       | Replace Conditional with Polymorphism(255):State 模式                                  |

### 重构存在问题：

1. 大多数重构都会减少代码总量，但是本次却增加了代码总量。
2. 性能

- 重构时不需要担心这些，优化时才要担心它们，但那时候你已处于一个比较有利的位置，有更多选择可以完成有效优化。（见第 69 页的讨论）
