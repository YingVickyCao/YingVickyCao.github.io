## 第3章 代码的坏味道 —— Kent Beck和Martin Fowler
...
- 如果同一个类的两个函数还有相同的表达式：提炼出重复代码。
-  如果两个互为兄弟的子类内含有相同的表达式：提炼相同代码，并放到父类中。
-  如果两个互为兄弟的子类内只是代码间相似，并非完全相同：将相似部分和差异部分拆开，构成单独的函数。然后使用模板方法【P345】的设计模式。
- 如果两个毫不相关的类中出现重复代码：可以将重复代码提炼成一个函数放到一个独立类中或者只放在某一个类中（总之要放在合适的地方），然后其他类都去调用这个函数。
